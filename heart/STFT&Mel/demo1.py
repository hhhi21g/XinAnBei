...
# ========== 参数设置 ==========
input_dir = "..\\dataSet_original\\lhb"  # 音频文件夹
output_dir = ("..\\envelope_images\\0")  # 所有图像统一输出文件夹
os.makedirs(output_dir, exist_ok=True)

features_dir = "..\\features\\0"
...
#         import pickle
#
#         pickle.dump(user_mfcc_segments, f)
#
...
# ========= 单文件处理函数 =========
def process_single_audio_file(file_path, draw_images=False, image_output_dir=None):
    y, sr = librosa.load(file_path, sr=None)
    y = y[int(5 * sr):]  # 去除前5秒
    ...
    for sec in range(duration):
        ...
        # 绘图（可选）
        if draw_images and image_output_dir:
            os.makedirs(image_output_dir, exist_ok=True)
            time_sec = np.linspace(0, 1, end - start)
            plt.figure(figsize=(6, 2))
            plt.plot(time_sec, envelope_centered[start:end])
            plt.axis("off")
            save_path = os.path.join(image_output_dir, f"{name_only}_sec_{sec + 1:03d}.png")
            plt.savefig(save_path, bbox_inches='tight', pad_inches=0)
            plt.close()
        ...
        try:
            ...
            print(f"❌ MFCC 提取失败：{name_only}_sec_{sec+1}，错误：{e}")

    return np.array(mfcc_segments)  # shape: (秒数, N_MFCC)

mfcc_array = process_single_audio_file(
    file_path="..\\dataSet_original\\lhb\\record15.m4a",
    draw_images=True,
    image_output_dir="..\\process"
)

print("提取 MFCC 段数：", len(mfcc_array))
print("每段 MFCC 维度：", mfcc_array.shape[1])
